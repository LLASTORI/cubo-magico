/**
 * PROMPT 29: Aba Fluxos - Caminhos reais dos clientes (visual)
 * 
 * Pergunta que responde:
 * "Qual caminho os clientes realmente percorrem?"
 * 
 * Contém:
 * ✔️ Visualização de fluxo (Sankey-like)
 * ✔️ Filtros por entrada/destino
 * ✔️ Configuração de passos e mínimo de clientes
 * 
 * NÃO contém:
 * ❌ Total de contatos
 * ❌ LTV
...
export function CustomerFlowsAnalysis() {
  ...
  const { data: transactions = [], isLoading } = usePaginatedQuery<TransactionData>(
    ['crm-flows-transactions', projectId],
    {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Visualização dos caminhos percorridos pelos clientes
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground">
              Dados insuficientes para visualizar fluxos.
            </p>
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p>Visualiza o caminho que os clientes percorrem entre produtos. A largura das linhas indica a quantidade de clientes.</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </CardTitle>
            <CardDescription>
              {customersWithMultiplePurchases.toLocaleString('pt-BR')} clientes com múltiplas compras
            </CardDescription>
          </div>
