/**
 * Experience Completion Settings
 * 
 * Unified completion settings component used by both Quiz and Survey.
 */

import { Gift, Clock, Plus, Trash2, ExternalLink, MousePointer } from 'lucide-react';
...
import { ExperienceCompletionConfig, CTAButton, DEFAULT_COMPLETION_CONFIG } from './types';

interface ExperienceCompletionSettingsProps {
  settings: ExperienceCompletionConfig;
  onChange: (settings: ExperienceCompletionConfig) => void;
  type: 'quiz' | 'survey';
}

export function ExperienceCompletionSettings({
  settings,
  onChange,
  type,
}: ExperienceCompletionSettingsProps) {
  const currentSettings = { ...DEFAULT_COMPLETION_CONFIG, ...settings };
  
  const updateSettings = (updates: Partial<ExperienceCompletionConfig>) => {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Adicione botões para direcionar o usuário após concluir o {typeLabel}
          </CardDescription>
        </CardHeader>
