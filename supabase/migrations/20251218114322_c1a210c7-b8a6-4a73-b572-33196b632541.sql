-- Fix Hotmart→CRM sync trigger: align ON CONFLICT target with existing unique constraint
-- Existing unique constraint on crm_transactions: UNIQUE (project_id, platform, external_id)
...
BEGIN
    ...
    IF v_contact_id IS NOT NULL AND NEW.status NOT IN ('ABANDONED') THEN
        ...
        -- IMPORTANT: match existing unique constraint: (project_id, platform, external_id)
