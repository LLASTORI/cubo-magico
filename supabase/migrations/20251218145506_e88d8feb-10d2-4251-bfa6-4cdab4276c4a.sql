...
-- Atualizar função para detectar recuperação de carrinhos abandonados
...
BEGIN
  ...
  -- Verificar se tinha carrinho abandonado antes (para compras aprovadas)
  IF NEW.status IN ('APPROVED', 'COMPLETE') THEN
    -- Verificar se existe um registro de abandono anterior para este email/projeto
    SELECT EXISTS(
  ...
  IF NEW.status IN ('APPROVED', 'COMPLETE') THEN
    ...
    -- Se tinha carrinho abandonado, adicionar tag de recuperação
