...
-- Atualizar função para remover tag genérica 'Carrinho Abandonado' quando todos os abandonos foram recuperados
...
BEGIN
  ...
  IF NEW.status IN ('APPROVED', 'COMPLETE') THEN
    ...
    -- NOVO: Verificar se ainda existem abandonos pendentes (tags 'abandonou:' sem compra correspondente)
    -- Se não houver mais abandonos pendentes, remover a tag genérica 'Carrinho Abandonado'
    v_has_remaining_abandonments := false;
    FOR i IN 1..array_length(v_new_tags, 1) LOOP
      IF v_new_tags[i] LIKE 'abandonou:%' THEN
        v_has_remaining_abandonments := true;
        EXIT;
      END IF;
    END LOOP;
    
    -- Se não há mais abandonos pendentes, remover tag genérica
    IF NOT v_has_remaining_abandonments THEN
