
-- Deduplicar sales_core_view por transaction_id
-- Prioridade: COMPLETE > APPROVED > BACKFILL > outros
DROP VIEW IF EXISTS public.sales_core_view;

CREATE VIEW public.sales_core_view AS
WITH ranked_events AS (
  SELECT 
    sce.id,
    sce.project_id,
    sce.provider,
    sce.provider_event_id,
    sce.event_type,
    sce.gross_amount,
    sce.net_amount,
    sce.currency,
    sce.occurred_at,
    sce.received_at,
    sce.economic_day,
    sce.contact_id,
    sce.is_active,
    sce.created_at,
    CASE
      WHEN sce.provider = 'hotmart' THEN (regexp_match(sce.provider_event_id, 'hotmart_([A-Z0-9]+)_'))[1]
      ELSE sce.provider_event_id
    END AS transaction_id,
    hs.buyer_name,
    hs.buyer_email,
    hs.product_name,
    hs.product_code,
    hs.offer_code,
    hs.status AS hotmart_status,
    om.funnel_id,
    f.name AS funnel_name,
    hs.utm_source,
    hs.utm_campaign_id AS utm_campaign,
    hs.utm_adset_name AS utm_adset,
    hs.utm_placement,
    hs.utm_creative,
    ROW_NUMBER() OVER (
      PARTITION BY sce.project_id, 
        CASE
          WHEN sce.provider = 'hotmart' THEN (regexp_match(sce.provider_event_id, 'hotmart_([A-Z0-9]+)_'))[1]
          ELSE sce.provider_event_id
        END
      ORDER BY
        CASE hs.status
          WHEN 'COMPLETE' THEN 1
          WHEN 'APPROVED' THEN 2
          WHEN 'BACKFILL' THEN 3
          ELSE 4
        END,
        sce.occurred_at DESC
    ) AS rn
  FROM sales_core_events sce
  LEFT JOIN hotmart_sales hs 
    ON hs.project_id = sce.project_id 
    AND hs.transaction_id = (regexp_match(sce.provider_event_id, 'hotmart_([A-Z0-9]+)_'))[1]
  LEFT JOIN offer_mappings om 
    ON om.project_id = sce.project_id 
    AND om.codigo_oferta = hs.offer_code
  LEFT JOIN funnels f 
    ON f.id = om.funnel_id
)
SELECT 
  id,
  project_id,
  provider,
  provider_event_id,
  event_type,
  gross_amount,
  net_amount,
  currency,
  occurred_at,
  received_at,
  economic_day,
  contact_id,
  is_active,
  created_at,
  transaction_id,
  buyer_name,
  buyer_email,
  product_name,
  product_code,
  offer_code,
  hotmart_status,
  funnel_id,
  funnel_name,
  utm_source,
  utm_campaign,
  utm_adset,
  utm_placement,
  utm_creative
FROM ranked_events
WHERE rn = 1;

GRANT SELECT ON public.sales_core_view TO anon, authenticated;
